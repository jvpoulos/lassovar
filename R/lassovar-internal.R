...
.lassovar.eq <-
function(y,x,ada.w,degf.type=NULL,ic,mc=FALSE,ncores=1,alpha=1,dfmax,nlambda,standardize,trend)
{
	...
	if(!mc){for(i in 1:ncol(y)){ all.ic[[i]]	<-.lv.eq.gn(i,y,x,ada.w,ic=ic,alpha=alpha,dfmax=dfmax,nlambda=nlambda,standardize=standardize,trend)}}
	if(mc){	all.ic<-mclapply(1:ncol(y),.lv.eq.gn,y,x,ada.w,ic=ic,alpha=alpha,dfmax=dfmax,nlambda=nlambda,standardize=standardize,trend,mc.cores=ncores)}
...
.lv.eq.gn<-function(i,y,x,ada.w,ic,alpha,dfmax,nlambda,standardize,trend)
{
...
# Computing exclusion vectors.	
  ...
  # Plain lasso
	if(is.null(ada.w)){
		...
		gn.mod	<-glmnet(x=x,y=y[,i],family='gaussian',exclude=all.excl,penalty.factor=wpen,alpha=alpha,dfmax=dfmax,nlambda=nlambda,standardize=standardize,type.gaussian='covariance')}
	...
	if(!is.null(ada.w)){
		...
		if(sum(ada.w$w[,i]==Inf)==length(ada.w$w[,i])){
			...
			gn.mod	<-glmnet(x=x,y=y[,i],family='gaussian',exclude=c(all.excl,which(ada.w$w[,i]==Inf)),penalty.factor=wpen,alpha=alpha,dfmax=dfmax,nlambda=nlambda,standardize=FALSE,type.gaussian='covariance')}
	}
