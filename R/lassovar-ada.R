...
.ada.las.weights<-function(y,x,ada,ic='BIC',mc=NULL,ncores,dfmax,nlambda,standardize,trend)
{
	ada.w	<-list('ada'=ada)
	gamma	<-1
	if(is.null(mc))mc<-FALSE

	lv.las	<-.lassovar.eq(y=y,x=x,degf.type=NULL,ada.w=NULL,ic=ic,mc=mc,ncores=ncores,alpha=1,dfmax=dfmax,nlambda=nlambda,standardize=standardize,trend=trend)
...
.ada.ridge.weights<-function(y,x,ada,ic='BIC',mc=NULL,ncores,dfmax,nlambda,standardize,trend)
{
	ada.w	<-list('ada'=ada)
	gamma	<-1
	if(is.null(mc))mc<-FALSE

	lv.las	<-.lassovar.eq(y=y,x=x,degf.type=NULL,ada.w=NULL,ic=ic,mc=mc,ncores=ncores,alpha=0,dfmax=dfmax,nlambda=nlambda,standardize=standardize,trend=trend)
